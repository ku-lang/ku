...
- 插件化：将编译流程抽象出来，做成可以添加插件的模式。除了最基本的语言要素外，其他语言特性都做成编译器插件的形式。
  - 未来可以扩展支持第三方语言插件
  - 可以通过配置不同插件来实现不同的语言
...
- 语言开发环境：模仿浏览器的DevTool，开发一个可以动态展示与操作AST树（及其他编译中间结果）的工具。配合REPL的话，语言开发的流程可以与前端JS开发类比，大大提高开发效率。
  - 要做到这一点，需要一个类似DomTree的统一AST树接口。

- 图形化的Debug工具。在LLDB的基础上，实现一个可视化的Debug工具。可以独立运行，也集成到IDE中。

- 深入研究LLVM，优化编译代码，提高运行速度。
